...
// test can read it without a DOM, and the answer must be the same for a file
// whose grammar chunk has not loaded yet. Whoever resolves the name downstream
// looks it up as a word, so what is picked here has to be a word.
 ...
 * `openFile|` are the same request. It reaches no further than the string it
 * is handed — callers pass one line — so a name never spans a line break.
 *
 * A run that starts with a digit is not a name (`0xff`, `42`) and is refused
 * here rather than sent on to be refused downstream.
 */
