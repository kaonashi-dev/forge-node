...
export type WorkbenchView =
  ...
  | { kind: "pr_compose" }
  /**
   * One pull request being reviewed by an agent.
   *
...
export function retargetViewPath(path: string, from: string, to: string): string {
  return retargetPath(path, from, to);
}
...
export function retargetView(view: WorkbenchView, from: string, to: string): WorkbenchView {
  ...
  const path = retargetViewPath(view.path, from, to);
