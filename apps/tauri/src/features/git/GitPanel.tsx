  ...
  return (
    <div class="panel-body">
      ...
      <div class="panel-heading">
        Changes
        <Show when={gitStore.diff}>
          <span class="panel-note-inline">
            +{diffTotals(gitStore.diff).additions} −{diffTotals(gitStore.diff).deletions}
          </span>
        </Show>
        <Show when={activeWorkspace() && (gitStore.diff?.files.length ?? 0) > 0}>
          <Button
            variant="secondary"
            size="xs"
            onClick={() => {
      ...
      <For
        each={gitStore.diff?.files}
        fallback={
          <p class="empty-copy">{loading.diff ? "Reading the diff…" : "Nothing uncommitted."}</p>
        }
      >
        {(file) => (
          <div class="git-file">
      ...
      <div class="git-actions">
        ...
        <Button variant="secondary" disabled={!gitStore.diff?.files.length} onClick={openDiff}>
          Open patch
        </Button>
      </div>
    </div>
