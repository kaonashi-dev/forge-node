...
import { createWorktree, fetchRemote } from "./commands";
import { launchAgent, launchShell } from "../sessions/sessionActions";
import { forgeStore } from "../../state/forgeStore";
import { projectDialogsStore, setProjectDialogsStore } from "./dialogs";
import { Button, Combobox, Dialog, Select, TextField, type ComboboxOption } from "../../ui/index";
...
import { listBranches } from "../git/commands";
...
export function BranchPicker(props: BranchPickerProps) {
  ...
  // Use the combobox selection so the footer button and Enter choose the same branch.
  ...
  const entries = createMemo(() => branchEntries(query(), gitStore.branches, forgeStore));
  ...
  function choose(entry: BranchPickerEntry | null | undefined): void {
    ...
    setError(null);
    setProjectDialogsStore("worktreeCreationFailure", null);
    setPending(branch);
  ...
  createEffect(() => {
    const failure = projectDialogsStore.worktreeCreationFailure;
  ...
  const hasRemote = () => (gitStore.branches?.remotes.length ?? 0) > 0;
  const branchesLoading = () => loading.branches;
  ...
  return (
    ...
    >
      <Combobox
        ...
        empty={
          <p class="empty-copy">{branchesLoading() ? "Loading branches…" : "No branches match."}</p>
        }
        leading={
          <>
