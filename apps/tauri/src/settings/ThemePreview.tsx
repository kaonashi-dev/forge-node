...
export function ThemePreview(props: { contrast: number }) {
  const [view, setView] = createSignal<"code" | "terminal">("code");
    ...
    <section class="theme-preview" aria-label="Theme preview" style={styles()}>
      <div class="theme-preview-toolbar">
        ...
        <div role="group" aria-label="Preview mode">
          <Button size="sm" selected={view() === "code"} onClick={() => setView("code")}>
            Code
          </Button>
          <Button size="sm" selected={view() === "terminal"} onClick={() => setView("terminal")}>
            Terminal
          </Button>
        </div>
        <span class="theme-preview-filename">{view() === "code" ? "theme.ts" : "zsh"}</span>
      </div>
      <Show
        when={view() === "code"}
        fallback={
