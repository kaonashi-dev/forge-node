...
import { focusWorkspace, setLoading, workbenchStore } from "../store/workbenchStore";
...
describe("Git decoration refresh", () => {
  ...
  it("stops on a failed read and lets a later change retry", async () => {
    ...
    expect(workbenchStore.loading.diff).toBe(false);
    expect(workbenchStore.diffError).toBe("diff unavailable");
    ensureDiff();
    await vi.advanceTimersByTimeAsync(15_000);
    expect(reads).toEqual(["w1"]);
    failRead = false;
  ...
  it("ignores a late send failure after leaving and reopening the same checkout", async () => {
    ...
    await vi.advanceTimersByTimeAsync(0);
    expect(workbenchStore.loading.diff).toBe(true);
    expect(workbenchStore.diffError).toBeNull();
  });
});
