...
export type Session = {
  ...
  /**
   * The launch profile this session ran with.
   *
...
export type ProviderInfo = {
  descriptor?: {
    ...
    prompt?: unknown;
    /** The provider's own read-only posture: its flags and what it
     ...
     *  Absent means it has no such switch, so a profile for it can
...
/**
 * Whether this provider can be sent to review a pull request.
 *
...
export type Launchable = {
  ...
  key: string;
  /**
   * Whether this row can start with an initial prompt.
   *
...
/** Discovered history, not a daemon-owned session. */
export type ExternalAgentSession = {
   ...
   * one. Resuming has to pass it back: the run only exists for a CLI
...
export type PullRequest = {
   ...
   *  filtering by them requires no further request. */
...
export type ShellSnapshot = {
  ...
  /**
   * Launch profiles for the settings section that edits them.
   *
  ...
  /**
   * Every project's file-sharing rules, in application order.
   *
  ...
  /**
   * Every project's worktree-ignore rules, for the dialog that undoes
  ...
  /**
   * Persisted GUI preferences. The daemon stays authoritative: writes
...
/**
 * What removing a project does to its sessions and worktrees.
 *
