//! The authoritative VT engine contract, driven by the daemon (ADR-011).
//! Engines consume PTY bytes and expose snapshots and damage to
//! [`crate::snapshot::DeltaBuilder`]; the client keeps only a passive replica.
//! The trait is always available; the Alacritty implementation is feature-gated.

use std::ops::Range;

use domain::{Cursor, Damage, PtySize, Row, TermModes, TerminalSnapshot};

/// The authoritative terminal engine.
///
/// Implementations are `Send` because each terminal's engine lives behind the
/// runtime lock and is driven from a dedicated PTY thread.
pub trait TerminalEngine: Send {
    ...
    /// scrollback rows. Sent on attach and resync.
    ...
    // These two are read-only accessors for values already modeled on
    ...
    /// the `seq` carried by snapshots and deltas.
    fn seq(&self) -> u64;
